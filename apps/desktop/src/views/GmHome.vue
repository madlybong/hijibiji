  ...
  <div class="min-h-screen bg-surface-950 p-8 flex flex-col items-center">
    <div class="w-full max-w-5xl">
      ...
      <div class="flex items-center gap-4 mb-12 select-none">
        ...
        <h1 class="text-2xl font-montserrat font-bold tracking-widest text-surface-0">GOLDMINE</h1>
      </div>

      <div class="flex justify-between items-end mb-6">
        <h2 class="text-xl font-bold text-surface-0">Recent Documents</h2>
      ...
      <div v-if="docs.length === 0" class="flex flex-col items-center justify-center p-16 border border-dashed border-surface-700 rounded-xl bg-surface-900/50">
        <FileText class="w-12 h-12 text-surface-500 mb-4" />
        <p class="text-surface-300">No documents found. Create one to get started.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <Card v-for="doc in docs" :key="doc.id" class="bg-surface-900 border border-surface-700 hover:border-primary/50 transition-colors cursor-pointer" @click="handleOpen(doc.id)">
          ...
          <template #subtitle>
            <div class="text-xs text-surface-400 truncate">{{ doc.description || 'No description' }}</div>
          </template>
          <template #content>
            <div class="text-[10px] text-surface-500 mt-4">
